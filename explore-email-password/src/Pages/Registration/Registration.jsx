...
export default function Registration() {
    ...
    const handleRegistration = (e) => {
        ...
        if (terms === false) {
            setErrorMessage('please accepet this terms');
            return;
        } 
        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then(result => {
                ...
                .then(() => {
                        alert('please login your email account and verify address') 
                    })
            })
            .catch(error => {
                console.log(error)
                ...
                <div className="card-body">
                    ...
                    <form onSubmit={handleRegistration} className="">
                        ...
                        <div className="flex gap-3 m-3">
                            ...
                            <p className="">please accepet our terms and condition</p>
                        </div>
                        <input className='btn w-full' type="submit" value="Submit" />
                        <p className='mt-5 text-center'>Already have an account? Please <Link to="/login" className='text-blue-500 underline'>Login</Link></p>
                    </form>
                    {
                        errorMessage && <p className='text-red-500 text-center'>{errorMessage}</p>
                    }
                    {
                        success && <p className='text-green-500 text-center'>user has been success</p>
                    }
                </div>
            </div>
        </div>
